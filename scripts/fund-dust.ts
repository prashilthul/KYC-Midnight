...
function getMnemonicFromArgs(): string {
    ...
    const printUsage = () => {
        console.error(`
Usage:
  npm run fund-dust "<mnemonic words>"

Accepted inputs:
  • BIP-39 mnemonic (space-separated words)

Example:
  npm run fund-dust "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about"
`);
    };
...
async function registerDustGeneration(
    ...
    // Wait for dust balance to be > 0
    logger.info('Waiting for dust balance to appear...');
    const dustBalance = await rx.firstValueFrom(
        walletFacade.state().pipe(
            rx.filter((s) => {
                const bal = s.dust.walletBalance(new Date());
                logger.debug({ bal }, 'Checking dust balance...');
                return bal > 0n;
            }),
    ...
    logger.info({ dustBalance: dustBalance.toString() }, 'Receiver dust balance after generation');
...
async function main(): Promise<void> {
    ...
    try {
        ...
        console.log('\n✅ DUST GENERATION COMPLETE!');
        console.log('You can now use your wallet for transactions.');
